...
impl NodeEvidence {
    ...
    ///
    /// The main-search form: requires a real score and `min_depth` plies of
    /// stored depth. Rarog's accepted callers pass 0, admitting depth-0 qsearch
    /// entries; RAR-S22 measured those as 67% of stores. The argument remains
    /// useful to targeted tests and future evidence consumers, but it is not a
    /// user-facing tuning coordinate.
    #[inline(always)]
...
mod tests {
    ...
    fn only_the_main_search_form_enforces_a_depth_floor() {
        ...
        // The accepted main-search depth floor of 0 admits a depth-0 entry.
