...
| RAR-R11 | **A.3.3 time-forfeit repair at `3+0.03` - REGISTERED, NOT YET RUN.** Baseline: the A.3.1 head (`ca8988a`, PGO pext, 1.98.1). Candidate: the same head plus the A.3.3 fix (clock-proportional low-time reserve at every thread count, hard stop measured on wall time including the bestmove write; exact diff recorded in the row before the run). `tools/sprt.ps1 -Mode fixed -Games 10000 -NoAdjudication`, `3+0.03`, 1T, Hash 64, paired UHO, concurrency 14 with affinity, `timemargin` 20 ms, `Move Overhead` 10 ms on both arms. Forfeits counted per arm from the log and cross-checked against the reconstructed PGN clocks. | **Prediction, frozen 2026-09-09:** baseline arm 4 to 12 forfeits in 10,000 games (the measured rate is 0.05 to 0.13%); candidate arm 0 to 2; paired Elo **-1 to +1** (a 30-40 ms reserve at a three-second clock costs about 1% of thinking time on the last moves only). Probability the fix halves the rate: 0.7. Confidence moderate on direction, low on the exact counts at these rates. | Accept only if the candidate forfeits at most a quarter of the baseline's count AND the paired interval excludes -3 Elo; a rate reduction bought with more than that is a harness setting (`Move Overhead`), not an engine change. RAR-M14 and the reconstructed clocks of the seven forfeited games (2026-09-09) are the diagnosis; the engine spends its whole budget by its own accounting and the forfeit is the wall-time gap under a saturated host. Time management is bench-invisible; games decide. | PLAN A.3.3; RAR-M14; `analysis/` record to be written by A.3.3(1); `tools/results/sprt_e16cand_vs_e16base232_20260909_170716.pgn` game 130 and the six earlier forfeits |
